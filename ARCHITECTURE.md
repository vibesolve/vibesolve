...
| **Provider-agnostic agent caller** | A common `BaseAgentCaller` interface fronts any-llm. `--provider` accepts any installed any-llm provider name; `claude` remains an alias for `anthropic`. Per-provider model IDs and reasoning efforts live under `provider_models.<provider>.<agent>`, with an optional `_default` key per provider block. Custom providers must supply a default model or cover every agent explicitly. |
